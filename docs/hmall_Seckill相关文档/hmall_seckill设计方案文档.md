...
> 整合 hmall 枫叶商城秒杀功能的全部设计方案，涵盖 C 端秒杀模块和管理后台秒杀管理两部分。

---

## 目录

- [第一部分：C 端秒杀模块设计](#第一部分c-端秒杀模块设计) — 秒杀系统架构 / 数据模型 / 秒杀流程 / 缓存策略 / Lua 脚本 / 接口设计
- [第二部分：管理后台秒杀管理设计](#第二部分管理后台秒杀管理设计) — 后台秒杀活动管理 / 商品关联 / 订单查询 / 库存查看
