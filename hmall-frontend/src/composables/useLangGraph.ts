 ...
 *
 * 封装 @langchain/langgraph-sdk Client，管理对话状态，
 * 提供 sendMessage / resume / clearHistory 方法。
 *
 * 支持 customer_agent 和 admin_agent 两种 Agent。
 */
...
export function useLangGraph(options: UseLangGraphOptions) {
  ...
  /** 处理 SSE 流式响应 */
  async function _processStream(streamResponse: AsyncIterable<any>) {
    ...
    for await (const chunk of streamResponse) {
      ...
      if (chunk.event === 'error') {
        error.value = chunk.data?.message || 'Agent 处理失败'
        break
      }
    }
  }
